...
public class SpecialContractApi {
	...
	private List<Node> nodeCall(Web3j web3j,BigInteger blockNumber,int funcType) throws Exception {
        ...
        if(br==null||br.data==null){
            throw new Exception("查询历史节点合约出错: 入参(blockNumber="+blockNumber+",funcType="+funcType);
        }
        if(br.isStatusOk()){
            String data = br.data;
            ...
            throw new Exception("调用合约失败:"+br.errMsg);
        }
    }
    ...
    public List<RestrictingBalance> getRestrictingBalance(Web3j web3j, String addresses) throws Exception {
        ...
        if(br==null||br.data==null){
            throw new Exception("查询锁仓计划合约出错: 入参(addresses="+addresses);
        }
        if(br.isStatusOk()){
            String data = br.data;
            if("".equals(data) && data==null){
	...
	public ProposalParticiantStat getProposalParticipants ( Web3j web3j, String proposalHash, String blockHash)throws Exception{
        ...
        if(br==null||br.data==null){
            throw new Exception("查询提案参与人出错: 入参(proposalHash="+proposalHash+",blockHash="+blockHash);
        }
        if(br.isStatusOk()){
            String data = br.data;
            if(!"".equals(data) && data!=null){
            ...
            if (a.length<4) throw new Exception("返回数据不完整!");
